...
void MeshCollider::Create(void* pData, const shared_ptr<void> gameObject)
{
...
void MeshCollider::DebugDraw(IDirect3DDevice9* pd3dDevice)
{
	...
	else
	{
		...
		pIndiceVec = _gameObject->GetComponent<MeshRenderer>()->GetMesh()->GetSubMesh(0)->GetIndice();
...
void MeshCollider::createBulletMesh(const shared_ptr<Mesh> pMeshContainer)
{
	...
	for (int i = 0; i < subMeshCount; ++i)
	{
		...
		pIndiceVec = pMeshContainer->GetSubMesh(i)->GetIndice();
		size_t indiceCount = pMeshContainer->GetSubMesh(i)->GetPrimitiveCount();
